...
```bash
./src/groestlcoind -signet -daemon
```
...
for ((n=1;n<=3;n++))
do
 ./src/groestlcoin-cli -signet -named createwallet wallet_name="participant_${n}" descriptors=true
done
```
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./src/groestlcoin-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./src/groestlcoin-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
external_desc_sum=$(./src/groestlcoin-cli -signet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./src/groestlcoin-cli -signet getdescriptorinfo $internal_desc | jq '.descriptor')
...
Documentation for these and other parameters can be found by typing `./src/groestlcoin-cli help importdescriptors`.
...
```bash
./src/groestlcoin-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true descriptors=true

./src/groestlcoin-cli  -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./src/groestlcoin-cli  -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./src/groestlcoin-cli -signet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./src/groestlcoin-cli -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./src/groestlcoin-cli -a $receiving_address
```
...
```bash
./src/groestlcoin-cli -signet -rpcwallet="multisig_wallet_01" getbalances
```
...
```bash
balance=$(./src/groestlcoin-cli -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./src/groestlcoin-cli -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./src/groestlcoin-cli -signet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./src/groestlcoin-cli -signet decodepsbt $funded_psbt

./src/groestlcoin-cli -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./src/groestlcoin-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./src/groestlcoin-cli -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./src/groestlcoin-cli -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./src/groestlcoin-cli -signet finalizepsbt $combined_psbt | jq -r '.hex')

./src/groestlcoin-cli -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./src/groestlcoin-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./src/groestlcoin-cli -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./src/groestlcoin-cli -signet finalizepsbt $psbt_2 | jq -r '.hex')
