...
On unix systems, the `--enable-multiprocess` build option can be passed to `./configure` to build new `groestlcoin-node`, `groestlcoin-wallet`, and `groestlcoin-gui` executables alongside existing `groestlcoind` and `groestlcoin-qt` executables.
...
- [ ] Adding `-ipcbind` and `-ipcconnect` options to `groestlcoin-node`, `groestlcoin-wallet`, and `groestlcoin-gui` executables so they can listen and connect to TCP ports and unix socket paths. This will allow separate processes to be started and stopped any time and connect to each other.
- [ ] Adding `-server` and `-rpcbind` options to the `groestlcoin-wallet` executable so wallet processes can handle RPC requests directly without going through the node.
...
make
src/groestlcoin-node -regtest -printtoconsole -debug=ipc
GROESTLCOIND=groestlcoin-node test/functional/test_runner.py
```
