...
`./`               | `banlist.dat`         | Stores the addresses/subnets of banned nodes (deprecated). `groestlcoind` or `groestlcoin-qt` no longer save the banlist to this file, but read it on startup if `banlist.json` is not present.
