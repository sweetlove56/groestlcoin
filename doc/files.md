...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Groestlcoin blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `groestlcoin.conf`    | User-defined [configuration settings](bitcoin-conf.md) for `groestlcoind` or `groestlcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
...
These subdirectories and files are no longer used by the Groestlcoin Core:
