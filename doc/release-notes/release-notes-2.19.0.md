...
Groestlcoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Groestlcoin Core on unsupported systems.
...
Users running macOS Catalina need to "right-click" and then choose "Open"
to open the Groestlcoin Core .dmg.
...
- [Reduce memory](https://github.com/groestlcoin/groestlcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Groestlcoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 1 GiB of space.  Note: this version of Groestlcoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `groestlcoind -help` output for these two options for
...
- To eliminate well-known denial-of-service vectors in Groestlcoin Core,
  ...
  This prevents Groestlcoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Groestlcoin Core and nodes that have manually enabled BIP37 support,
...
- By default, Groestlcoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `groestlcoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Groestlcoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Groestlcoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Groestlcoin Core will be bound to
  the standard port 1331 even if a different port is configured for
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Groestlcoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  GRS), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in GRS per 1,000
  vbytes, not gro per vbyte. (#16257)
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Groestlcoin Core v2.17.2), this can eliminate a
