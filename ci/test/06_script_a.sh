...
if [ -n "$ANDROID_TOOLS_URL" ]; then
  ...
  DOCKER_EXEC ./configure "$GROESTLCOIN_CONFIG" --prefix="${DEPENDS_DIR}/aarch64-linux-android" || ( (DOCKER_EXEC cat config.log) && false)
  DOCKER_EXEC "make $MAKEJOBS && cd src/qt && ANDROID_HOME=${ANDROID_HOME} ANDROID_NDK_HOME=${ANDROID_NDK_HOME} make apk"
  exit 0
fi

GROESTLCOIN_CONFIG_ALL="--enable-suppress-external-warnings --disable-dependency-tracking --prefix=$DEPENDS_DIR/$HOST --bindir=$BASE_OUTDIR/bin --libdir=$BASE_OUTDIR/lib"
if [ -z "$NO_WERROR" ]; then
...
DOCKER_EXEC "${BASE_ROOT_DIR}/configure" --cache-file=config.cache "$GROESTLCOIN_CONFIG_ALL" "$GROESTLCOIN_CONFIG" || ( (DOCKER_EXEC cat config.log) && false)

DOCKER_EXEC make distdir VERSION="$HOST"

export P_CI_DIR="${BASE_BUILD_DIR}/groestlcoin-$HOST"

DOCKER_EXEC ./configure --cache-file=../config.cache "$GROESTLCOIN_CONFIG_ALL" "$GROESTLCOIN_CONFIG" || ( (DOCKER_EXEC cat config.log) && false)
