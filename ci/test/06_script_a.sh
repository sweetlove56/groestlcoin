...
if [ -n "$ANDROID_TOOLS_URL" ]; then
  ...
  CI_EXEC ./autogen.sh
  CI_EXEC ./configure "$GROESTLCOIN_CONFIG" --prefix="${DEPENDS_DIR}/aarch64-linux-android" || ( (CI_EXEC cat config.log) && false)
  CI_EXEC "make $MAKEJOBS && cd src/qt && ANDROID_HOME=${ANDROID_HOME} ANDROID_NDK_HOME=${ANDROID_NDK_HOME} make apk"
  exit 0
fi

GROESTLCOIN_CONFIG_ALL="--enable-external-signer --enable-suppress-external-warnings --disable-dependency-tracking --prefix=$DEPENDS_DIR/$HOST --bindir=$BASE_OUTDIR/bin --libdir=$BASE_OUTDIR/lib"
if [ -z "$NO_WERROR" ]; then
...
CI_EXEC "${BASE_ROOT_DIR}/configure" --cache-file=config.cache "$GROESTLCOIN_CONFIG_ALL" "$GROESTLCOIN_CONFIG" || ( (CI_EXEC cat config.log) && false)

CI_EXEC make distdir VERSION="$HOST"

export P_CI_DIR="${BASE_BUILD_DIR}/groestlcoin-$HOST"

CI_EXEC ./configure --cache-file=../config.cache "$GROESTLCOIN_CONFIG_ALL" "$GROESTLCOIN_CONFIG" || ( (CI_EXEC cat config.log) && false)
