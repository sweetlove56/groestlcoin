This directory contains the GroestlcoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
ClientModel is used by the main application `groestlcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/bitcoin/bitcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a groestlcoin:... link from a browser).
...
10. Start debugging with Qt Creator (you might need to the executable to "groestlcoin-qt" under "Run", which is where you can also add command line arguments)
