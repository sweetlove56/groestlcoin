    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (!::ChainstateActive().IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            ...
            if (pindex->nVersion != 112 && pindex->nVersion > VERSIONBITS_LAST_OLD_BLOCK_VERSION && (pindex->nVersion & ~nExpectedVersion) != 0) //GRS
...
bool LoadMempool(CTxMemPool& pool)
{
    ...
    try {
        ...
        try {
          ...
          // mempool.dat files created prior to v2.21 will not have an
