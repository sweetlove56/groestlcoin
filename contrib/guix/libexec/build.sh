...
make -C depends --jobs="$JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform groestlcoin-linux-g++' \
                                   qt_config_opts_x86_64_linux='-platform linux-g++ -xplatform groestlcoin-linux-g++' \
...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} GROESTLCOIN_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac
