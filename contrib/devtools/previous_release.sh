  ...
  do
    ...
    if [ "$DOWNLOAD_BINARY" -eq "0" ]; then
      ...
      if [ ! -d "$tag" ]; then
        ...
        git clone https://github.com/groestlcoin/groestlcoin "$tag"
        pushd "$tag" || exit 1
        {
          git checkout "$tag"
          ...
          mkdir bin
          mv src/groestlcoind src/groestlcoin-cli src/groestlcoin-tx bin
          if [ "$FUNCTIONAL_TESTS" -eq "0" ]; then
            mv src/qt/groestlcoin-qt bin
          fi
        }
        popd || exit 1
      fi
      ...
      if [ -d "$tag" ]; then
        ...
        if [[ "$tag" =~ v(.*)(rc[0-9]+)$ ]]; then
            BIN_PATH="Groestlcoin/groestlcoin/releases/download/v${BASH_REMATCH[1]}/test.${BASH_REMATCH[2]}"
        else
            BIN_PATH="Groestlcoin/groestlcoin/releases/download/v${tag:1}"
        fi
        URL="https://github.com/$BIN_PATH/groestlcoin-${tag:1}-$PLATFORM.tar.gz"
        echo "Fetching: $URL"
        if ! curl -O -f $URL; then
            echo "Download failed."
            exit 1
        fi
        tar -zxf "groestlcoin-${tag:1}-$PLATFORM.tar.gz" -C "$tag" --strip-components=1 "groestlcoin-${tag:1}"
        rm "groestlcoin-${tag:1}-$PLATFORM.tar.gz"
      fi
    fi
  done
}
